...
resource "tls_cert_request" "opa_server_cert_request" {
  dns_names       = [kubernetes_service.opa.load_balancer_ingress["hostname"], "${kubernetes_service.opa.metadata.0.name}.${kubernetes_namespace.opa_namespace.metadata.0.name}.svc"]
  key_algorithm   = "RSA"
